...
from ddtrace.internal.processor.profiling import ProfilingSpanProcessor
...
def _get_span_processor():
    profiling_span_processor = ProfilingSpanProcessor()
    profiling_span_processor.endpoint_counts = _ENDPOINT_COUNTS
    return profiling_span_processor
...
def test_wrong_api_key(endpoint_test_server):
    ...
    exp = http.PprofHTTPExporter(
        ...
        max_retry_delay=2,
        profiling_span_processor=_get_span_processor(),
    )
...
def test_export(endpoint_test_server):
    exp = http.PprofHTTPExporter(endpoint=_ENDPOINT, api_key=_API_KEY, profiling_span_processor=_get_span_processor())
...
def test_export_404_agent(endpoint_test_unknown_server):
    exp = http.PprofHTTPExporter(endpoint=_UNKNOWN_ENDPOINT, profiling_span_processor=_get_span_processor())
    with pytest.raises(exporter.ExportError) as t:
        exp.export(test_pprof.TEST_EVENTS, 0, 1)
    assert str(t.value) == (
...
def test_export_404_agentless(endpoint_test_unknown_server):
    exp = http.PprofHTTPExporter(
        endpoint=_UNKNOWN_ENDPOINT, api_key="123", timeout=1, profiling_span_processor=_get_span_processor()
    )
    with pytest.raises(exporter.ExportError) as t:
        exp.export(test_pprof.TEST_EVENTS, 0, 1)
