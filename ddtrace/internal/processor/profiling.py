...
class ProfilingSpanProcessor(SpanProcessor):
    ...
    _endpoint_counts_lock = attr.ib(init=False, repr=False, factory=nogevent.DoubleLock, eq=False)
    ...
    def on_span_finish(self, span):
        ...
        if not self._enabled:
            return
        if span._local_root == span and span.span_type == SpanTypes.WEB and span.resource is not None:
